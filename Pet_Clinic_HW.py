...
class Owner(Base):
    ...
    id: Mapped[int] = mapped_column(int, unique=True, primary_key=True)
    name: Mapped[String] = mapped_column(str(150), unique=True, nullable=False)
    phone:  Mapped[int] = mapped_column(str(25), unique=True, nullable=False)
...
class Pet(Base):
    ...
    breed: Mapped[str] = mapped_column(str(150), nullable=False)
    owner_id: Mapped[int] = mapped_column(int(150), unique=True, ForeignKey('Owners.id'))
